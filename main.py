...
if __name__ == "__main__":
    ...
    startup_dict = __import__(config.get('Programs', 'startup').strip()).communicate(
        {'input_filename': "state_defs/" + config.get('General', 'state_def_filename')})
    ...
    signal.signal(signal.SIGINT,
                  signal.SIG_IGN)  # A ignore interupt is required, otherwise child processes will inherit the interupt.
    inp = mul.Process(target=reader.get_data, args=(reader_out_q,))
    p1 = mul.Process(target=__import__(config.get('Programs', 'J1939_Interpreter').strip()).communicate,
                     args=(reader_out_q, J1939_Interpreter_out_q, {'startup': startup_dict['to_J1939_Interpreter']},))
    p2 = mul.Process(target=__import__(config.get('Programs', 'state_indexer').strip()).communicate,
                     args=(J1939_Interpreter_out_q, state_indexer_out_q, startup_dict['to_state_indexer'],))
    p3 = mul.Process(target=__import__(config.get('Programs', 'visualizer').strip()).communicate,
                     args=(state_indexer_out_q, visualizer_out_q, startup_dict['to_visualizer'],))
