def state_def_interpreter(inp_file):
	...
	for states in states_from_file:
		...
		for def_dict in states['definition']:
			...
			pgn_h = "{:04X}".format(def_dict['pgn'])
			da_h = "{:02X}".format(def_dict['da'])
			sa_h = "{:02X}".format(def_dict['sa'])
			if int(pgn_h[:2],16) < 240: #if pf < 240, mask da into pgn
				pgn_h = pgn_h[:2] + da_h
			pgn_h = pgn_h + sa_h	
			
			#Next prepare the pgn spn map
			tup = pgn_spn_map.get(pgn_h,(def_dict['pgn'],def_dict['da'],def_dict['sa'],set()))
			tup[3].add(def_dict['spn'])
			pgn_spn_map[pgn_h] = tup
			
			#Next for all the stuff to he sent to the state-indexer
			#For state_parameter_map
			state_switch_dict[(def_dict['pgn'],def_dict['da'],def_dict['sa'],def_dict['spn'])] = 0			
			#For parameter_state_map
			psmap = parameter_state_map.get((def_dict['pgn'],def_dict['da'],def_dict['sa'],def_dict['spn']),({},{}))
			psmap[0][state_name] = 0
			#For parameter_state_map values
			for item in def_dict['pinst']:
			...
			parameter_state_map[(def_dict['pgn'],def_dict['da'],def_dict['sa'],def_dict['spn'])] = psmap
	...
	return {'to_da_interpreter': pgn_spn_map,'to_state_indexer':{'parameter_state_map':parameter_state_map,'state_parameter_map':state_parameter_map},'to_visualizer':state_visualizer_map}#, to_gateway': (arb_sa_mask_16,int(arb_sa_mask_16,16))} 
	
def DA_interpreter(pgn_spn_map):
	#Get J1939_DA cursor
	import sqlite3, copy, re, math
	...
	for pgn_h,spn_map in pgn_spn_map.items():
		tup = []
		for SPNData in cur.execute('SELECT SPN,SPNPOsitioninPGN,SPNLength,Resolution,Offset FROM SPNandPGN WHERE SPNLength <> "" and PGNDOC = "J1939DA" and MultiPacket = "No" AND PGN='  +str(spn_map[0]) + '  AND SPN in (' + ",".join(str(i) for i in list(spn_map[3]))+ ')') :       
			...
			tup.append(((spn_map[0],spn_map[1],spn_map[2],int(SPNData[0])),mask,resolution,offset,possplit[0] -1))
